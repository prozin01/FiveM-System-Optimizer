...
if %errorlevel% neq 0 (
    ...
    powershell -Command "Start-Process powershell -ArgumentList '-NoProfile -ExecutionPolicy Bypass -Command \"cd /d %cd% && python GUI-Optimizer.py\"' -Verb RunAs"
    exit /b
)
REM Se chegou aqui, ja tem permissao de admin
echo [OK] Executando como Administrador...
timeout /t 1 >nul
python GUI-Optimizer.py
